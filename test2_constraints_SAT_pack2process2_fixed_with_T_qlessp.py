...
constraint13,constraint14a,constraint14b,constraint15,constraint15a = Bools('constraint13 constraint14a constraint14b constraint15 constraint15a')
...
s.add(Implies(constraint1a,T==2))
        ...
        for b in range(1, 3):
            s.add(Implies(constraint17a,              
                         Implies(And(knows(t+1,p,b) ==True, knows(t,p,b)==False),#if process p knows a packet now and did not know it in the previous step
                                 And(Exists(q,And(q>=0,q<p,transmit(t,q)== b)), listen(t,p)==True,ForAll(q1, Implies(And(q1<p,q1>p),transmit(t,q1)==-1)))
                                 )                     
                  )
          )
